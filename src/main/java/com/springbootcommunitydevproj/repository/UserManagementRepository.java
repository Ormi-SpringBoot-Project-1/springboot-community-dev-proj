...
public interface UserManagementRepository extends JpaRepository<User, Integer> {
    ...
    // 회원 리스트를 가져오기 위한 조건절 없는 select 쿼리문입니다.
    // user, level, post 세 테이블을 Join합니다.
    ...
    // start offset을 통해 한 페이지 당 최대 10개씩 조회합니다.
    // (3/28 추가) id와 pageable로 어떤 열을 기준으로 오름차순/내림차순 정렬할 지 결정합니다. no offeset 으로 페이징합니다.
    @Query(value = SELECT_QUERY_WITHOUT_CONDITION
        + "where user.is_admin = 0 and user.user_id > :id group by user.user_id, user.nickname, level.level_name", nativeQuery = true)
    List<UserManagementInfoDto> findAllUserManagementInfo(@Param("id") Integer id, Pageable pageable);
