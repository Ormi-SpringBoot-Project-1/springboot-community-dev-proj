...
public class UserManagementService {
    ...
    public List<UserManagementInfoDto> getAllUserManagementInfo(Integer page, String orderBy, String sort) {
        ...
        if (!(orderBy.equals("userId") || orderBy.equals("nickname")
            ...
            return userManagementRepository.findAllUserManagementInfo(pageRequest);
        }

        if (sort.equals("asc")) {
            pageRequest = PageRequest.of(page - 1, 10, Direction.ASC, orderBy);
        }
        else {
            pageRequest = PageRequest.of(page - 1, 10, Direction.DESC, orderBy);
        }
     ...
     */
    public Integer getUserManagementInfoPages() {
        return (int) Math.ceil((double) userManagementRepository.count() / 10);
    }

    /**
     *      회원의 닉네임을 기준으로 해당 회원 정보를 조회합니다.<br>
     *      nickname이 null이거나 조회 결과가 없으면 빈 리스트를 반환합니다.
     */
    ...
    /**
     *      가입 제한으로 설정된 회원의 user_id 정보를 가져옵니다. <br>
     *      Set으로 반환합니다.
     */
    public Set<Integer> getAllBlockedUser() {
